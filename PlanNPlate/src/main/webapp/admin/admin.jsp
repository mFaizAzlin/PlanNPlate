...
<body>
			...
			<div class="container">
				<div class="page-inner">
					...
					<div class="add-new-button">
						<a
							href="AdminController?action=add&adminID=<c:out value="${sessionId}" />"
					...
					<div class="row">
						<c:forEach items="${admins}" var="admin" varStatus="a">
							<div class="col-md-4">
								<div class="card">
									<div class="card-header text-center">
										<h4 class="card-title">${admin.adminID}</h4>
									</div>
									<div class="card-body">
										<div class="card card-profile">
											...
											<div class="card-body">
												<div class="user-profile text-center">
													...
													<div class="desc">${admin.quote}</div>
													<div class="social-media">
														<a class="btn btn-info btn-twitter btn-sm btn-link"> <span
															class="btn-label just-icon"><i
																class="icon-social-twitter"></i> </span>
														</a> <a class="btn btn-primary btn-sm btn-link"
															rel="publisher"> <span class="btn-label just-icon"><i
																class="icon-social-facebook"></i> </span>
														</a> <a class="btn btn-danger btn-sm btn-link" rel="publisher">
															<span class="btn-label just-icon"><i
																class="icon-social-instagram"></i> </span>
														</a>
													</div>
													<div class="view-profile">
														<a
															href="AdminController?action=view&viewID=<c:out value="${admin.adminID}"/>&adminID=<c:out value="${sessionId}" />"
															class="btn btn-secondary w-100"> View Full Profile</a>
													</div>
												</div>
											</div>
											<div class="card-footer d-flex justify-content-center">
												<input type="hidden" id="adminID-${a.index}"
													value="<c:out value="${admin.adminID}"/>"> 
												<input
													type="hidden" id="sessionID-${a.index}"
													value="<c:out value="${sessionId}"/>">

												<button class="btn btn-danger"
													onclick="confirmation('${a.index}')">delete</button>
											</div>
										</div>
									</div>
								</div>
	...
	<script>
		function confirmation(index) {
			...
			var sessionID = document.getElementById('sessionID-' + index).value
			...
			if (confirm('Are you sure you want to delete this admin? This action cannot be reversed.')) {
				window.location.href = 'AdminController?action=delete&adminID=' + adminID + '&sessionID=' + sessionID;
			}
		}
	</script>
